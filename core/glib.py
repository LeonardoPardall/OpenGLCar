...
# re-export OpenGL symbols into module namespace without using wildcard import
for _name in dir(gl):
    if _name.startswith('gl') or _name.startswith('GL_'):
        globals()[_name] = getattr(gl, _name)
import ctypes
import os
import numpy as np
...
class Pipeline:
    def __init__(self, program, vertex_format: list[tuple[int, int, int, int]]):
...
def wrapperCreateShader(name: str) -> ShaderProgram:
    ...
    candidate_pkg = os.path.join(base_dir, 'shaders')
    candidate_root = os.path.join(base_dir, '..', 'shaders')
    if os.path.isdir(candidate_pkg):
        shaders_dir = candidate_pkg
    elif os.path.isdir(candidate_root):
        shaders_dir = os.path.abspath(candidate_root)
    else:
        # fallback to package path (will raise if missing)
        shaders_dir = candidate_pkg

    vs_path = os.path.join(shaders_dir, f'{name}.vert')
    fs_path = os.path.join(shaders_dir, f'{name}.frag')
